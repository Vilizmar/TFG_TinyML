...
#include "SDRAM.h"
#include <TFG_SENDA_FINAL_inferencing.h>
#include "camera.h"
#include "himax.h"
...
//Fase de prototipado en Web Socket Client
//const char* mqtt_server = "broker.hivemq.com";
//Fase final con el despliegue de docker
const char* mqtt_server = "10.109.178.61";
...
// Auxiliar function in case it disconnects unexpectedly
void reconnectMQTT() {
  while (!mqttClient.connected()) {
    ei_printf("Intentando conexión a HiveMQ...\n");
    ...
    if (mqttClient.connect("PortentaTFG_Nodo1_Test")) {
      ...
      ei_printf(" intentando de nuevo en 5 segundos\n");
      delay(5000);
    }
  }
}
...
void setup()
{
    ...
    // Esto pausa el programa hasta que abramso el monitor serial (así vemos todos los print)
    // Si está alimentada la placa por pila hay que quitarlo
    while (!Serial); 
    
    ei_printf("\n--- INICIANDO SISTEMA PORTENTA ---\n");

    // Hay que inicializar la SDRAM ya que WIFI + Deteccion de objetos es muy pesado
    #ifdef EI_CAMERA_FRAME_BUFFER_SDRAM
        SDRAM.begin(); 
        delay(100);
        ei_printf("SDRAM inicializada correctamente.\r\n");
    #endif
    ...
    // Inicializamos el wifi después de la cámara puesto que sino podría haber problemas de fragmentación de memoria
    ei_printf("Conectando a red WiFi %s...\n", ssid);
    ...
    // Inciializamos la conexión MQtt con un timeout de 15 segundos si no llega a conectarse.
    mqttClient.setBufferSize(256);
    mqttClient.setServer(mqtt_server, mqtt_port);
    ei_printf("Conectando al broker MQTT HiveMQ...\n");
    ...
    if (!mqttClient.connected()) {
        ei_printf("\nERR: Tiempo de espera MQTT agotado. Reiniciando...\n");
        delay(1000);
        NVIC_SystemReset(); // Reinicio forzad
    }

    // Validaciones de Edge Impulse de si la imagen es en blanco y negro (creo que es innecesario en este caso)
    for (size_t ix = 0; ix < ei_dsp_blocks_size; ix++) {
        ...
        if (block.extract_fn == &extract_image_features) {
            ...
            if (channel_count == 3) {
                ei_printf("WARN: Has desplegado un modelo en color, pero la cámara es monocroma.\r\n");
                break;
            }
        }
    }

    ei_printf("\n--- SETUP COMPLETADO. Comenzando inferencia... ---\n");
    delay(1000);
}
...
void loop()
{
    ...
    //We reconnect if we lose the connection
    if (!mqttClient.connected()) {
        reconnectMQTT();
    }
    ...
    for (uint32_t i = 0; i < result.bounding_boxes_count; i++) {
        ...
        if (bb.value <= 0.0 || bb.label == nullptr) {
            continue;
        }

        char payload[128] = {0};
        const char* etiqueta_final = bb.label;


        if (strncmp(bb.label, "persona", 7) == 0) {
            if (bb.x < 48) {
                etiqueta_final = "persona_fuera";
            } else {
                etiqueta_final = "persona_dentro";
            }
        }

        snprintf(payload, sizeof(payload), "{\"clase\":\"%s\", \"confianza\":%.2f}", bb.label, bb.value);
        ...
        snprintf(payload, sizeof(payload), "{\"clase\":\"%s\", \"confianza\":%.2f}", etiqueta_final, bb.value);

        // Pub the message on topic 
        if (mqttClient.connected()) {
            mqttClient.publish("tfgsenda/vlizmar/nodo1", payload);
        }
    }
